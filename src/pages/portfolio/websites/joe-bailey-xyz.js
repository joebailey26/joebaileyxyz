              ...
              <article key={node.title} id={node.slug} className="with_featureImage">
                ...
                <footer>
                  <div className="buttonsContainer">
                    <Link className="btn" to={"portfolio/websites/joe-bailey-xyz/" + node.slug}>Continue Reading</Link>
                  </div>
...
export const pageQuery = graphql`
  query {
    ...
    allWordpressPost(filter: {categories: {elemMatch: {name: {eq: "Joe Bailey XYZ"}}}, status: {eq: "publish"}}, sort: { fields: [date] order: DESC }) {
        edges {
          node {
            title
            excerpt
